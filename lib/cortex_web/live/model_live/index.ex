defmodule CortexWeb.ModelLive.Index do
  use CortexWeb, :live_view

  alias Cortex.Config
  alias Cortex.Config.{LlmModel, Settings, Metadata}
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok,
     ...
     |> assign(:page_title, "Model Management")}
  end
  ...
  defp apply_action(socket, :new, _params) do
    ...
    |> assign(:model, %LlmModel{source: "custom", status: "active", enabled: true})
  end
  ...
  def handle_event("toggle_enabled", %{"id" => id}, socket) do
    ...
    socket =
      if socket.assigns.live_action == :edit && socket.assigns.model.id == id do
        assign(socket, :model, Config.get_llm_model!(id))
      else
        socket
      end
  ...
  def handle_event("edit_model", %{"id" => id}, socket) do
    {:noreply, push_patch(socket, to: ~p"/settings/models/#{id}/edit")}
  end
  ...
  def render(assigns) do
    ...
    <main class="flex-1 overflow-y-auto relative h-full">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="sm:flex sm:items-center sm:justify-between mb-8">
          <div>
            <h1 class="text-3xl font-bold text-white">Model 管理</h1>
          ...
          <div class="mt-4 sm:mt-0 flex space-x-3">
            ...
            >
              <.icon name="hero-plus" class="-ml-1 mr-2 h-4 w-4" /> 新建 Model
            </.link>
          </div>
        </div>
    ...
    <!-- Filters -->
        <div class="mb-6 flex space-x-4 bg-slate-800 p-4 rounded-lg border border-slate-700">
          <form phx-change="filter" class="flex space-x-4 w-full">
            ...
            <tbody class="bg-slate-800 divide-y divide-slate-700">
              <tr :for={model <- @models} class="hover:bg-slate-700/50 transition-colors">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  ...
                  >
                    {if model.enabled, do: "已启用", else: "已禁用"}
                  </button>
                </td>
  ...
  defp model_modal(assigns) do
    ...
    >
      ...
      <div
        ...
        aria-hidden="true"
        phx-click={JS.patch(~p"/settings/models")}
      >
      </div>
