defmodule CortexWeb.SettingsLive.Channels do
  ...
  defp load_configs(socket) do
    ...
    |> assign(:page_title, "Channel Settings")
    # Default tab
  ...
  def handle_event("save", params, socket) do
    {:noreply,
     save_channel_config(params, socket, &load_configs/1)}
  end
  ...
  def render(assigns) do
    ...
    <!-- Tab Content -->
          <div class="bg-slate-900 border border-slate-800 p-8 rounded-xl">
            <%= case @active_tab do %>
              <% :telegram -> %>
                <.telegram_form configs={@configs} />
              <% :feishu -> %>
                <.feishu_form configs={@configs} />
              <% _ -> %>
                <.telegram_form configs={@configs} />
            <% end %>
          </div>
